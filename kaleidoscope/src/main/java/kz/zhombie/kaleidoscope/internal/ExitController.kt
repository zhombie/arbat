...
class ExitController constructor(
    view: View,
    ...
    private val scrollThresholdScaled: Float = UnitsUtils.toPixels(view.context, SCROLL_THRESHOLD)

    private val animatorView: AnimatorView? = if (view is AnimatorView) view else null
    ...
    fun isExitDetected(): Boolean = isScrollDetected || isZoomDetected
    ...
    fun onScroll(dx: Float, dy: Float): Boolean {
        ...
        if (!skipScrollDetection && !isExitDetected() && canDetectExit()
            && canDetectScroll() && !canScroll(innerDy)
        ) {
            totalScrollX += dx
            totalScrollY += innerDy

            // Waiting until we scrolled enough to trigger exit detection or to skip it
            if (abs(totalScrollY) > scrollThresholdScaled) {
                isScrollDetected = true
                initialY = controller.getState().getY()
                startDetection()
            } else if (abs(totalScrollX) > scrollThresholdScaled) {
        ...
        if (isScrollDetected) {
            ...
            if (scrollDirection == 0f) {
                scrollDirection = sign(innerDy)
            }

            // Gradually decreasing scrolled distance when scrolling beyond exit point
            if (exitState < EXIT_THRESHOLD && sign(innerDy) == scrollDirection) {
                innerDy *= exitState / EXIT_THRESHOLD
            }
            ...
            exitState = 1f - (controller.getState().getY() + innerDy - initialY) / total
            ...
            if (exitState == 1f) {
                ...
                controller.getState().translateBy(0f, innerDy)
            }
    ...
    private fun isZoomedOut(): Boolean {
        val state: State = controller.getState()
    ...
    private fun startDetection() {
        ...
        if (controller is GestureControllerForPager) {
            controller.disableViewPager(true)
        }
    }
        ...
        if (isExitDetected()) {
            ...
            if (animator?.isAnimating() == false && canDetectExit()) {
                ...
                if (isLeaving) {
                    ...
                    if (position < 1f) {
                        animator.setState(pos = position, leaving = false, animate = true)
    ...
    private fun updateState() {
        if (canDetectExit()) {
            ...
            animatorView?.getPositionAnimator()?.setState(pos = exitState, leaving = false, animate = false)
        }
    }
